...
export default function CreateWorkflow() {
  ...
  const handleSave = async () => {
    ...
    try {
      if (!workflowId) {
        const created = await apiFetch<{ _id: string }>("/api/workflows", {
          method: "POST",
          body: JSON.stringify({
      ...
      setSaveState({ tone: "success", message: "Saved to backend (and kept local draft)." });
    } catch {
      setSaveState({ tone: "warning", message: "Saved locally. Backend API was not reachable or you are signed out." });
    }
  };
  ...
  return (
    <div className="space-y-6">
      <Card className="rounded-2xl border-white/70 bg-white/75 p-5 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div className="flex items-start gap-3">
            <div className="mt-1 flex h-10 w-10 items-center justify-center rounded-xl bg-stone-950 text-amber-300">
              <Workflow size={18} />
            </div>
            <div className="min-w-0">
              <p className="text-xs font-semibold uppercase tracking-[0.24em] text-stone-500">Builder</p>
              <div className="mt-2 grid gap-3 lg:grid-cols-2">
                <div className="space-y-1">
                  <Label>Workflow Name</Label>
                  <Input className="border-stone-300 bg-stone-50" value={workflowName} onChange={(event) => setWorkflowName(event.target.value)} />
                </div>
                <div className="space-y-1">
                  <Label>Description</Label>
                  <Input className="border-stone-300 bg-stone-50" value={workflowDescription} onChange={(event) => setWorkflowDescription(event.target.value)} />
                </div>
              </div>
              <p className="mt-3 text-sm text-stone-600">
                Trigger: <span className="font-medium text-stone-900">{triggerSummary}</span> · Actions:{" "}
                <span className="font-medium text-stone-900">{actionCount}</span>
              </p>
      ...
      <Card className="overflow-hidden rounded-2xl border-white/70 bg-white/65 shadow-sm backdrop-blur">
        <div className="flex flex-col gap-4 border-b border-stone-200 px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            <span className="hidden text-sm text-stone-500 lg:inline">Tip: drag from the right handle into empty canvas.</span>
          </div>
        </div>
        ...
        <SheetContent side="left" className="w-full border-r border-stone-200 bg-[#f7efe4] p-0 sm:max-w-xl">
          <div className="h-full overflow-y-auto p-6">
            <SheetHeader>
              ...
              <SheetDescription>When the canvas is empty, the builder starts here. Pick the event that should start the workflow.</SheetDescription>
            </SheetHeader>
      ...
      <Sheet open={Boolean(selectedNode)} onOpenChange={(open) => !open && setSelectedNodeId(null)}>
        <SheetContent side="right" className="w-full border-l border-stone-200 bg-[#fcfaf6] sm:max-w-xl">
          <SheetHeader>
            <SheetTitle className="font-serif text-3xl">{selectedNode?.type === "trigger" ? "Edit trigger" : "Edit action"}</SheetTitle>
            <SheetDescription>Update node settings from this side sheet. Changes are applied directly to the canvas.</SheetDescription>
          </SheetHeader>

          <div className="mt-8">
            {selectedNode?.type === "trigger" && (
              <TriggerConfig data={selectedNode.data as TriggerNodeData} onUpdate={(newData) => updateNodeData(selectedNode.id, newData)} />
            )}
            {selectedNode?.type === "action" && (
              <ActionConfig data={selectedNode.data as ActionNodeData} onUpdate={(newData) => updateNodeData(selectedNode.id, newData)} />
            )}
          </div>
