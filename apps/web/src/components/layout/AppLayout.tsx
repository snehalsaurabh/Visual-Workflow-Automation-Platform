...
export default function AppLayout() {
  ...
  const signedIn = Boolean(typeof window !== "undefined" && authStore.getToken());
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,_rgba(251,191,36,0.28),_transparent_24%),radial-gradient(circle_at_bottom_right,_rgba(251,146,60,0.18),_transparent_28%),linear-gradient(180deg,_#f6f0e8_0%,_#f3ede4_56%,_#ece5da_100%)] text-stone-900">
      <div className="mx-auto max-w-7xl px-4 pb-8 pt-4 sm:px-6 lg:px-8">
        <header className="sticky top-4 z-40 mb-8 rounded-2xl border border-white/70 bg-white/75 px-4 py-4 shadow-sm backdrop-blur xl:px-6">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            <nav className="flex flex-wrap items-center gap-2">
              ...
              {!signedIn ? (
                ...
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-full bg-stone-100 px-4 py-2 text-sm text-stone-700 transition hover:bg-stone-200"
                >
                  <LogIn size={16} />
                  Signout
                </button>
            ...
            <div className="rounded-full border border-stone-200 bg-stone-50 px-4 py-2 text-sm text-stone-600">
              User: <span className="font-semibold text-stone-900">{getCurrentUser()}</span>
            </div>
          </div>
        </header>
